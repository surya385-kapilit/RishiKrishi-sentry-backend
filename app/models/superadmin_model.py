...
def get_superadmin_by_email(email):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT email, password, role, full_name FROM superadmins WHERE email = %s", (email,))
            return cur.fetchone()
    finally:
...
def get_superadmin_by_id(superadmin_id):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT superadmin_id, email, role, full_name, created_at FROM superadmins WHERE superadmin_id = %s", (superadmin_id,))
            return cur.fetchone()
    finally:
        db_pool.putconn(conn)
...
def get_all_superadmins():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT superadmin_id, email, role, full_name, created_at FROM superadmins ORDER BY created_at DESC")
            return cur.fetchall()
    finally:
        db_pool.putconn(conn)
...
def delete_superadmin(superadmin_id):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("DELETE FROM superadmins WHERE superadmin_id = %s", (superadmin_id,))
            conn.commit()
            return cur.rowcount > 0
    finally:
        db_pool.putconn(conn)
